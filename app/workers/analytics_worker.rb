class AnalyticsWorker
  ...
  #recurrence { minutely.second_of_minute(0, 30, 20, 30, 40, 50) }
  recurrence { hourly.minute_of_hour(4,30, 30, 45) }
  ...
  def data
    ...
    stats_count = stats()
    #json
    json_hash = {'thisWeek' => this_week, 'lastWeek' => last_week,'countries'=>countries,'os'=>os,'overview'=>overview,
    							'stats' =>stats_count,'thisYear' => this_year,'lastYear' => last_year,'visitors'=>visitors}
    #store in mongoDB
    @google_analytics = GoogleAnalytics.new
    #delete any records available
    @google_analytics.delete_all
    #add new record
    @google_analytics.name= "analytics"
    @google_analytics.json = json_hash
